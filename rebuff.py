...
def get_input_substrings(normalized_input: str, keyword_length: int) -> List[str]:
    ...
    for i in range(number_of_substrings):
        input_substrings.append(
            " ".join(words_in_input_string[i: i + keyword_length]))
...
def get_matched_words_score(
    ...
    matched_words_count = len(
        [part for part, word in zip(
            keyword_parts, substring.split()) if word == part]
    )

    if matched_words_count > 0:
        base_score = 0.5 + 0.5 * \
            min(matched_words_count / max_matched_words, 1)
    else:
        base_score = 0
...
print(detect_prompt_injection_using_heuristic_on_input(
